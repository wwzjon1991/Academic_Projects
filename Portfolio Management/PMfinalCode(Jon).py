...
#==============================================================================
# Code requires at least Pandas 0.22.0 to run
# Additional caclulated statistics have been commented out using ''' '''
#==============================================================================
...
#%% Performance Ratios for Normal Cumulative REturns Part 1A
''' 
def MaxDrawndown(xs):
...
#%% Performance Ratios for Equal Risk Contributions Portfolios
'''
...
def RollingVol(Optimal, Lookback, type):
    ...
    if type==12 or 6:
        ...
        Mktthreshold =  Optimal.loc[:'1990-01-01 00:00:00' ,'MKT-RF-Vol'].mean() \
                        +0*Optimal.loc[:'1990-01-01 00:00:00', 'MKT-RF-Vol'].std()                
    ...
    return Mktthreshold
Mktthreshold3 = np.float32(RollingVol(Opt_port3, 'Lookback 3 months', 3))
Mktthreshold6 = np.float32(RollingVol(Opt_port6, 'Lookback 6 months', 6))
Mktthreshold12 = np.float32(RollingVol(Opt_port12, 'Lookback 12 months', 12))
Mktthreshold24 = np.float32(RollingVol(Opt_port24, 'Lookback 24 months', 24))
Mktthreshold36 = np.float32(RollingVol(Opt_port36, 'Lookback 36 months', 36))
...
#print('Portfolio Volatility:',Opt_port36['EW'].dropna().std() )
    ...
    plt.fill_between(x, 0, ymax, where=BTPort['Regime'].dropna() < 1, facecolor='green', alpha=0.5, interpolate=True)
...
for num,BTPort in enumerate(Backtest):
    roll = BT[num]
    threshold = BacktestTH[num]
    ax = plt.plot(BTPort['Returns'].dropna().index,BTPort['Returns'].dropna().cumsum(),color='orange',label= str(roll) +' Lookback '+'Regime Portfolio')
